...
bool isCursorKeyPressed(int& whichKey)   //whichKey passed as reference.... 
{
	...
	if (key == 1)
	{
		...
		whichKey = 5; // 3 if enter key is pressed
		return true;
	}
	return false;
}
...
void myTriangle(int x1, int x2, int y,  int r, int g, int b)
{
	int mid = (x1 + x2) / 2;
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);
	...
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(r, g, b));
	SelectObject(device_context, pen);
	MoveToEx(device_context, mid, y-18, NULL);
	LineTo(device_context, x1, y);
	LineTo(device_context, x2, y);
	LineTo(device_context, mid, y-18);
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);

}
void myDimond(int x1, int x2, int y, int r, int g, int b)
{
	int mid = (x1 + x2) / 2;
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);
	...
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(r, g, b));
	SelectObject(device_context, pen);
	MoveToEx(device_context, mid, y - 10, NULL);
	LineTo(device_context, x1, y);
	LineTo(device_context, mid, y+10);
	LineTo(device_context, x2, y);
	LineTo(device_context, mid, y -10);
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
